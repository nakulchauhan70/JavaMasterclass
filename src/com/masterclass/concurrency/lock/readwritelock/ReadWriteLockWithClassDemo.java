...
class CalendarUserWithClass extends Thread {
    ...
    public CalendarUserWithClass(String name) {
        this.setName(name);
    }
...
public class ReadWriteLockWithClassDemo {
    public static void main(String[] args) {
        // create ten reader threads
        for (int i = 0; i < 10; i++)
            new CalendarUserWithClass("Reader-" + i).start();

        // ...but only two writer threads
        for (int i = 0; i < 2; i++)
            new CalendarUserWithClass("Writer-" + i).start();
    }
}