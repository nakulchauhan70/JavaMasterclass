package com.masterclass.concurrency.lock.trylock; /**
...
class Shopper extends Thread {
    ...
    public void run() {
        while (itemsOnNotepad <= 200) {
            if ((itemsToAdd > 0) && pencil.tryLock()) { // add item(s) to shared notepad
                try {
                    //Commenting pencil.tryLock() and using below pencil.lock() will take more time to execute
            ...
            } else { // look for other things to buy by another thread and it keeps on increasing count while first thread is writing above
                try {
